...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Enable Curve", m_curve->enabled);

	gui_1->addGroup("Animation Controls");
	gui_1->addVariable("Animation Enabled", m_aircraft_animation->enabled);
	gui_1->addVariable("t1", m_aircraft_animation->t1)->setMinMaxValues(0.0f, 1.0f);
	gui_1->addVariable("t2", m_aircraft_animation->t2)->setMinMaxValues(0.0f, 1.0f);
	gui_1->addVariable("Total time", m_aircraft_animation->total_moving_time)->setMinValue(0.1f);
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		if (obj_list[i].obj_name == "cube")
		{
			...
			for (unsigned int j = 0; j < m_curve->control_points_pos.size(); j++) {
				...
				glm::mat4 rotation = glm::toMat4(m_curve->control_points_quaternion[j]);
				...
				cur_obj_model_mat = translation * rotation * glm::inverse(translation) * cur_obj_model_mat;
		...
		if (obj_list[i].obj_name == "aircraft")
		{
			...
			if(m_aircraft_animation->enabled)
		...
		if (obj_list[i].obj_name == "curve" && m_curve->enabled) 
		{
			// Draw curve
